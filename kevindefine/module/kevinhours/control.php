...
class kevinhours extends control {
	...
	public function ajaxDeptEmployees($deptId) {
		...
		$html .= "<tr onclick='hideindeptmember(0);'><th class='text-center'><strong>&nbsp;ID</strong></th><th class='text-center'><strong>&nbsp;".$this->lang->kevinhours->indeptmember."</strong></th><th><strong class='pull-right'>&nbsp;".$this->lang->kevinhours->operate."</strong></th><th> <strong class='pull-right'><i class='icon-angle-right'></i></strong></th></tr>";
							...
							if(isset($name->isexternal)&&$name->isexternal==0){
								if(isset($name->locked)&&$name->locked==0){
									if ($key) {
										...
										$html .= "<tr id=$tempKey class='indeptmember text-center'><td>".($j + 1)."</td><td>".$name->realname."</td>";
										if ($dispatchedPriv) {
											$html .= "<td class='text-center'><input class='link' type='button' value='外协工作' onclick=dispatched('".$key."');></td>";
										}
										if ($printoverPriv) {
											$html .=  "<td class='text-center'><input class='link' type='button' value='班组加班' onclick=printover('".$key."');></td>";
										}
										$html .= '</tr>';
										$j++;
									}
						...
						if ($j == 0) $html .=  "<tr class='indeptmember'><td>".$this->lang->kevinhours->nothing.'</td><td></td><td></td><td></td></tr>';
						$j=0;
						$html .= "<tr class='text-right indeptmember' id ='InactiveUsers' onclick='hideInactive(0);'><td></td><td><td></td></td><td><i class='icon-angle-left'><strong><small>&nbsp{$this->lang->kevinhours->inactive}</small></strong></td></tr>";
							foreach ($employeesAll as $key => $name) {
								if(isset($name->isexternal)&&$name->isexternal==0){
									if(isset($name->locked)&&$name->locked==1){
										if ($key) {
											$tempKey = str_replace(".", "-", $key);
											$html .= "<tr id=$tempKey class='hideInactive text-center hidden'><td>".($j + 1)."</td><td class='indeptmember'>".$name->realname."</td>";
											if ($dispatchedPriv) {
												$html .=  "<td class='text-center'><input class='link' type='button' value='外协工作' onclick=dispatched('".$key."');></td>";
											}
											if ($printoverPriv) {
							...
							if($j==0)$html .= "<tr class='hideInactive hidden'><td class='indeptmember'>".$this->lang->kevinhours->nothing.'</td><td></td><td></td><td></td></tr>';
							$html .= "</table></div>";
							$j=0;
							$html .= "<div class='pull-left' style='width: 45%;margin-left: 2%;'><table class='table table-condensed  table-hover'>";
		
							$html .= "<tr onclick='hideindeptmember(1);'><th class='text-center'><strong>&nbsp;ID</strong></th><th class='text-center'><strong>&nbsp;".$this->lang->kevinhours->externalmember."</strong></th><th><strong class='pull-right'>&nbsp;".$this->lang->kevinhours->operate."</strong></th><th><strong class='pull-right'><i class='icon-angle-right'></i></strong></th></tr>";
							foreach ($employeesAll as $key => $name) {
								if(isset($name->isexternal)&&$name->isexternal==1){
									if(isset($name->locked)&&$name->locked==0){
										if ($key) {
											$tempKey = str_replace(".", "-", $key);
											$html .= "<tr id=$tempKey class='externalmember text-center'><td>".($j + 1)."</td><td>".$name->realname."</td>";
											if ($dispatchedPriv) {
												$html .=  "<td class='text-center'><input class='link' type='button' value='外协工作' onclick=dispatched('".$key."');></td>";
											}
											if ($printoverPriv) {
							...
							if($j==0)$html .= "<tr class='externalmember'><td>".$this->lang->kevinhours->nothing.'</td><td></td><td></td><td></td></tr>';
							$j=0;
							$html .= "<tr class='text-right externalmember' id ='InactiveUsers' onclick='hideInactive(1);'><td></td><td></td><td></td><td><i class='icon-angle-left'><strong><small>&nbsp{$this->lang->kevinhours->inactive}</small></strong></td></tr>";
							foreach ($employeesAll as $key => $name) {
								if(isset($name->isexternal)&&$name->isexternal==1){
									if(isset($name->locked)&&$name->locked==1){
										if ($key) {
											$tempKey = str_replace(".", "-", $key);
											$html .= "<tr id=$tempKey class='hidextInactive text-center hidden'><td>".($j + 1)."</td><td class='externalmember'>".$name->realname."</td>";
											if ($dispatchedPriv) {
												$html .=  "<td class='text-center'><input class='link' type='button' value='外协工作' onclick=dispatched('".$key."');></td>";
											}
											if ($printoverPriv) {
							...
							if($j==0)$html .= "<tr class='hidextInactive hidden'><td class='externalmember'>".$this->lang->kevinhours->nothing.'</td><td></td><td></td><td></td></tr>';
							$html .= "</table></div>";
		die($html);
//		die(html::select('userIndex', $this->kevinhours->getDeptEmployeesPairs($deptId), '', 'class=form-control'));
	...
	public function count($type = '', $account = '', $deptId = '', $hourstype = '', $employeetype = 'all', $recTotal = 0, $recPerPage = 20, $pageID = 1,$deptflag='x') {
		...
		if (!empty($_POST)) {
			...
			$account		 = isset($this->post->userIndex)? $this->post->userIndex : '';
		...
		if (common::hasPriv('kevinhours', 'checkAll')) {
			...
			$this->view->userIndex	 = $account ? $account:array_slice($this->kevinhours->getDeptEmployeesPairs($deptId), 1);
	...
	public function over($type = 'thisYear', $account = '', $deptID = '') {
		...
		$this->view->deptname = (isset($deptArray[$deptID]))?$deptArray[$deptID]:'Undifined';
		$deptchildmenu=array();
		$simplename=explode('/',$this->view->deptname);
...
//		$this->view->accountArrayInDept	 = $this->kevincom->getDeptEmployeesAccount($deptId);
	...
	public function product($productID = 0, $type = 'thisYear', $isShowDetail = 'false', $recTotal = 0, $recPerPage = 20, $pageID = 1) {
		...
		if($productID>0)$this->view->position[]	 = html::a($this->createLink($this->moduleName, 'browse'), $products[$productID]);
		$this->view->position[]	 = $this->lang->kevinhours->product;

		$this->commonWebParam($type);

		$this->kevinhours->productCountByProject($productID, $type, $pager);
	...
	public function project($projectID = 0, $type = 'thisYear', $isShowDetail = ''
	, $orderBy = 'date,status,begin', $recTotal = 0, $recPerPage = 20, $pageID = 1) {
		...
		if ($oldProjectID != 0 && $oldProjectID != $projectID) {
			if(commonModel::hasPriv('kevinhours', 'checkAll')){
		...
		$project	 = $this->project->getById($projectID);
                if (!$project){
                    if(isonlybody())die(js::error($this->lang->project->accessDenied) .js::reload ('parent.parent'));
                    die(js::error($this->lang->project->accessDenied) . js::locate('back'));
                }
		...
		$this->view->kevinhourscount = $this->loadModel('kevinhourscount');
		$kevinhourscount = & $this->view->kevinhourscount;//ref
		
		$kevinhourscount->ProjectItem = $project;
		//读取配置信息，是否显示详细人时
		if ($isShowDetail === 'true') {
			/* Load pager. */
			$this->app->loadClass('pager', $static	 = true);
			$pager	 = pager::init($recTotal, $recPerPage, $pageID);
			$kevinhourscount->todoGetListByProject($projectID, $type,$pager);
		}
		$rc = $kevinhourscount->CountProjectByAccout($this->view->begin,$this->view->end);//check
