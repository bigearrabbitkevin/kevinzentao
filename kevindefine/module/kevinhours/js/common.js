...
function getNewTodoListRecent(type)
{
	var link;
	if('project' === methodName){
		link = createLink('kevinhours', methodName, 'projectID='+currentProjectID + '&type='+type);
	}
	else if('product' === methodName){
		link = createLink('kevinhours', methodName, 'productID='+currentProductID + '&type='+type);
	}
	else{
		link = createLink('kevinhours', methodName, 'type='+type+'&account='+currentAccount+'&dept='+currentdept);
	}
	location.href=link;
}
