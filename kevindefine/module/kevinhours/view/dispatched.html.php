<!DOCTYPE html>
<?php
$ShowProjectName = isset($this->config->kevinhours->dispatchedShowProjectName)? $this->config->kevinhours->dispatchedShowProjectName:false;
...
$projectArray	 = array();//project array
$lastAccount	 = '';
$useritem = new stdclass();
//遍历代办
foreach ($todos as $todo):
	$projectArray[$todo->project] = $todo->project;
	//获得代办所有者数组,exist then add to array
	if ($lastAccount == $todo->account ) continue;
	$lastAccount	 = $todo->account;
	if($lastAccount)$accountArray[$lastAccount]	 = $lastAccount;
endforeach;

//查询所有的用户信息
$accountArray		 = $this->dao->select('a.*,b.name as deptName')->from(TABLE_USER)->alias('a')->leftjoin(TABLE_DEPT)->alias('b')
	->on('a.dept = b.id')
	->where('account')->in($accountArray)
	->fetchAll('account');

//查询所有的项目信息：
$projectArray	 = $this->dao->select('*')->from(TABLE_PROJECT)
	->where('id')->in($projectArray)
	->fetchAll('id');
	...
	<div class='container mw-1400px'>
	  <form method='post' id='todoform'>
		<center>
			<?php
			$i = 0;
			foreach ( $accountArray as $account =>$useritem) {
				$i++;
				?>
			  <body link="blue" vlink="purple">
				...
				<table width="800" border="1" cellpadding="0" cellspacing="0">
				  <tr height="35">
					...
					<td colspan="2" align="center"><font size='4'>&nbsp;<?php echo $useritem->deptName; ?></font></td>
					<th colspan="1" align="center"><font size='4'>姓名</font></th>
					<td colspan="1" align="center"><font size='4'><?php echo $useritem->realname; ?></font></td>
					<th colspan="2" align="center"><font size='4'>手机</font></th>
					<td colspan="2" align="center"><font size='4'><?php echo $useritem->mobile . '&nbsp;'; ?></font></td>
				  </tr>
				  <tr height="30">
					<th colspan="1" align="center"><font size='3' id='cardid'><strong>卡号</strong></font></th>
					<td colspan="2" align="center"><font size='4'>&nbsp;<?php echo $useritem->cardId; ?></font></td>
				  ...
				  $currentHours	 = 0;
				  foreach ($todos as $todo):
					if ($todo->account != $useritem->account) continue;
					$currentHours	 = $this->kevinhours->showHours($todo->minutes);
					$cashCode		 = array_key_exists($todo->project,$projectArray)? $projectArray[$todo->project]->cashCode:'0';
					//获得代办中项目代号数组
				...
				<?php if (1 == $this->config->kevinhours->ShowPartTable) { ?>
				<br>
				<table width="400" border="1" cellpadding="0" cellspacing="0">
				  <tr height="40">
					<th colspan="1" align="center"><font size='4'>项目零件</font></th>
					<th colspan="1" align="center"><font size='4'>分值</font></th>
				  </tr>
