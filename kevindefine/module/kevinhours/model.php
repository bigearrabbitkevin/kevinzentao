<?php
function ProjectDescHours($a, $b) {
	if ($a->totalHours == $b->totalHours)return 0;
...
class kevinhoursModel extends model {
	...
	public $ProductItem = null; //product Item
...
//				$hours	  = substr("" . ($todo->minutes / 60), 0, 4);
				...
				if('nor' == $hourtype)
				{
					if(array_key_exists($showDay, $norHoursArray)) $norHoursArray[$showDay] += $todo->minutes;
					else $norHoursArray[$showDay] = $todo->minutes;
				}
				else if('hol' ==$hourtype)
				{
					//请假
					if(array_key_exists($showDay, $holHoursArray)) $holHoursArray[$showDay] += $todo->minutes;
					else $holHoursArray[$showDay] = $todo->minutes;
				}
				else if('ann' == $hourtype)
				{
					//年假
					if(array_key_exists($showDay, $annHoursArray)) $annHoursArray[$showDay] += $todo->minutes;
					else $annHoursArray[$showDay] = $todo->minutes;
				}
				else if('rep' == $hourtype)
				{
					//调休
					if(array_key_exists($showDay, $excHoursArray)) $excHoursArray[$showDay] += $todo->minutes;
					else $excHoursArray[$showDay] = $todo->minutes;
				}
			}
			if(array_key_exists($showDay, $norHoursArray)){
				...
				if($totalnorhours!=480)return true;else return false;
			}else return false;
		}else return false;
	}
	...
	public function GetProjectByIDorCashCode($project) {
		...
		$String1 = "";
		$this->GetProjectCashcodeSearchString($String1);
	...
	//get search string
	public function GetProjectCashcodeSearchString(&$ioString) {
		$ioString = "(";
		$i = 0;
		foreach ($this->config->kevinhours->cashcodeProjects as $item){
			if($i>0) $ioString .= " or ";
			$ioString .= "(id < $item[1] and id>$item[0])";
			$i++;
		}
		if (0 == $i)$ioString = "";
		$ioString .= ")";
	...
	public function getLockedDate($year = '', $month = '') {
		$key = ($this->app->user->code == '')?'NoCode':'Code';
		$endDay = $this->config->kevinhours->endDay[$key];
		if($this->config->kevinhours->MonthLockEarly == $month) {
			//按照 endDayType == 0 来锁定
		} else 	if ($this->config->kevinhours->endDayType[$key] == 1 && !empty($this->kevincalendar)) { // working day
			$endDay = $this->kevincalendar->getWorkingDay($year, $month, $endDay);
		} else { //No. Day
			//按照 endDayType == 0 来锁定
		}
	...
	, $deptId = 0, $employeetype = '', $pager = null) {
		...
		if ($account == '') {
			...
			$stmt			 = $this->dao->select('c.cashCode,c.name as projectname,b.code,b.realname,a.*')->from(TABLE_TODO)->alias('a')
				...
				->andWhere("a.date <= '$end'")
				->andWhere("a.status = 'done'")
				->beginIF('formal' == $employeetype)->andWhere("b.code != ''")->fi()
				->beginIF('all' == $employeetype)->andWhere("b.code >= ''")->fi()
				->beginIF('all' != $employeetype && 'formal' != $employeetype)->andWhere("b.code <= ''")->fi()
				->beginIF('all' != $hourstype)->andWhere('hourstype')->eq($hourstype)->fi()
				->orderBy('a.account, a.date')
				->query();
		}
		else {
			$stmt	 = $this->dao->select('c.cashCode,c.name as projectname,b.code,b.realname,a.*')->from(TABLE_TODO)->alias('a')
...
//		}
	...
	public function getRecentProject() {
		...
		$projects			 = $this->dao->select("a.project, b.name AS projectName, max(`date`) as MaxDate")
			->from(TABLE_TODO)->alias('a')
			->leftJoin(TABLE_PROJECT)->alias('b')->on('a.project = b.id')
			->where('project')->ge(10)
			->andWhere('account')->eq($this->account)
	...
	public function getRelevantProject($keywords) {
		...
		$projects		 = $this->dao->select('id,name')
			->from(TABLE_PROJECT)
...
//		if(count($employees) == 1) return array();
	...
	public function overtreetitle($type = 'thisYear', $account = '', $deptID = '',$method,$userFunc,$params){
		...
		if(!isset($deptArray[$deptID])) return '';//返回空，不提示
		$deptchildmenu=array();
...
//		$this->CheckOutEmployeesPairs($deptID);
		...
		if($deptinfo){
			if($deptinfo->parent){ 
			if($method=='overdept')
				$parentstr="&nbsp;&nbsp;&nbsp;".html::a(helper::createLink('kevinhours', 'overdept', "type=$type&deptID=$deptinfo->parent"),"<i class='icon-level-up'></i>", '', "class='btn pull-center' id='gotoparent'");
			else
				$parentstr="&nbsp;&nbsp;&nbsp;".html::a(helper::createLink('kevinhours', $method, "type=$type&deptID=$deptinfo->parent"),"<i class='icon-level-up'></i>", '', "class='btn pull-center' id='gotoparent'");
			}
		}
		$deptchildmenustr='';
		...
		$treeHTML="<div class='side' style='width:".$this->config->kevinhours->sideWidth."px;'><a class='side-handle' data-id='kevinhoursTree' onclick='onsplice();'><i id='slipicon' class='icon-caret-left'></i></a><div class='side-body'><div class='panel panel-sm'><div class='panel-heading'><strong><ul class='nav'><li id='NextLinkOver'></li></ul>".$selectmenu."</strong></div><div class='panel-body table table-condensed  table-fixed' style = 'min-height:400px'><div class='nobr'><button class=' btn pull-left ".(($method=='over')? 'active':'')."' id='accountreverse' type='button' onclick='onButtonMember();'>".$this->lang->kevinhours->account."</button><button class=' btn  pull-left ".(($method=='overdept')? 'active':'')."' id='deptchildreverse' type='button' onclick='onButtonDeptChild();'>".$this->lang->kevinhours->dept."</button>".$parentstr."</div><table id='deptchildlist' class='hidden nav ".(($method=='over')? 'hidden':'')."'>".$deptchildmenustr."</table><table class='table table-condensed table-fixed table-hover table-striped ".(($method=='overdept')?'hidden':'')."' id='deptmember'>".$employmenu."</table></div></div></div></div><script>var mainmargin='".($this->config->kevinhours->sideWidth+20)."px';var width='".$this->config->kevinhours->sideWidth."px';if($('#slipicon').hasClass('icon-caret-right')){ $('.side').css('width',width);$('.main').css('margin-left',mainmargin);$('.main').resize();}else{ $('.side').css('width','');$('.main').css('margin-left','');$('.main').resize();}$('.hideInactive').addClass('hidden');$('.hidextInactive').addClass('hidden');function onButtonMember(){if($('#deptmember').hasClass('hidden')){ $('#deptchildlist').addClass('hidden');$('#deptmember').removeClass('hidden');$('#deptchildreverse').removeClass('active');$('#accountreverse').addClass('active');}else $('#deptchildlist').addClass('hidden');}function onButtonDeptChild(){if($('#deptchildlist').hasClass('hidden')){ $('#deptchildlist').removeClass('hidden');$('#deptmember').addClass('hidden');$('#deptchildreverse').addClass('active');$('#accountreverse').removeClass('active');}else $('#deptmember').addClass('hidden');}function onsplice(){if($('#slipicon').hasClass('icon-caret-right')){var width='".$this->config->kevinhours->sideWidth."px';var mainmargin='".($this->config->kevinhours->sideWidth+20)."px';$('.side').css('width',width);$('.main').css('margin-left',mainmargin);$('.main').resize();}else{ $('.side').css('width','');$('.main').css('margin-left','');$('.main').resize();}}
	...
	public function productCountByProject($productID, $type, $pager){
		$this->ProductItem = $this->loadModel('product')->getById($productID);
		if(!$this->ProductItem) return false;
		$ProductItem = &$this->ProductItem;//ref
		//list of todos
		$ListOfProject = $this->kevincom->getProjectsByProductID($productID);
		$stmt					 = $this->getProductStmtFromSql($productID, $type, $ListOfProject );
		$allTodos				 = $this->getTodosOfProduct($stmt);
		$ProductItem->todos = array();
		if ($pager != null) {
			$ProductItem->todos = $this->getTodosOfProduct($stmt, $pager);
		}

		$projects = $this->dao->select('*')->from(TABLE_PROJECT)->alias('a')
			->where('id')->in($ListOfProject)
			->orderBy('id')->fetchAll();
	
	
		$ProductItem->productHours				 = 0;//all
		$ProductItem->productforHours			 = 0;//正式
		$ProductItem->productinfHours			 = 0;//外援	
		$productHours				 = &$this->ProductItem->productHours;//all
		$productforHours			 = &$this->ProductItem->productforHours	;//正式
		$productinfHours			 = &$this->ProductItem->productinfHours;//外援
		$ProductItem->projects = array();
		//遍历项目数组
		foreach ($projects as $project) {
			$formalHours		 = 0;
			$informalHours		 = 0;
			...
			foreach ($allTodos as $todo):
				if(!($todo->hourstype  == 'nor' or $todo->hourstype  == 'ove'))continue;
			...
			$formalHours /=60.0;
			$informalHours /=60.0;
			$totalProductHours /=60.0;
			$totalHours /=60.0;
			if($totalHours <= 0) continue; //do not count 0 hours project 
			
			$productHours += $totalHours;
			$productforHours += $formalHours;
			$productinfHours += $informalHours;
			$formalHours	 = $this->showHours($formalHours, 2);
			$informalHours	 = $this->showHours($informalHours, 2);
			
			$totalPercent	 = (int)(($totalHours / $totalProductHours) * 1000)/10 . '%';

			$project->formalHours=$formalHours;
			$project->informalHours=$informalHours;
			$project->totalHours=$totalHours;
			$project->totalHoursShow=$this->showHours($totalHours*60, 2);
			$project->totalProductHours=$totalProductHours;
			$project->totalPercent=$totalPercent;
			$ProductItem->projects[$project->id] = $project;
			//die(json_encode($project));

		}
		usort($ProductItem->projects, 'ProjectDescHours');
	}
	...
	public function showWorkHours($iMinutes) {
		if ($iMinutes > 60) {
			if (0 == (int) ($iMinutes % 60))
