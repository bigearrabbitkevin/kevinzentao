	...
	<table class='table table-condensed table-hover table-striped tablesorter w-400px' id='kevinstoreCountList'>
		...
		<tbody>
			...
			foreach ($items as $item):
				...
				$xData .= "'" . (isset($lang->kevinstore->GroupTypeList[$item->groupName])?$lang->kevinstore->GroupTypeList[$item->groupName]:'') . "'";
				$yData .= $item->storeCount;
				?>
				<tr class='text-center'>
					<td><?php echo $i; ?></td>
					<td class='text-left'><?php echo isset($lang->kevinstore->GroupTypeList[$item->groupName])?$lang->kevinstore->GroupTypeList[$item->groupName]:''; ?></td>
